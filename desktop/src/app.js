...
(function () {
  const App = {
    ...
    getConfigDefs(type) {
      const defs = {
        trigger: [
          { key: "triggerType", label: "Trigger Type", type: "select", options: ["Webhook", "Schedule", "File Watch", "Manual"], default: "Manual" },
        ...
        action: [
          ...
          { key: "url", label: "URL / Endpoint", type: "text", default: "https://api.example.com" },
          { key: "method", label: "Method", type: "select", options: ["GET", "POST", "PUT", "DELETE"], default: "POST" },
        ],
        condition: [
          { key: "condition", label: "Condition Expression", type: "textarea", default: "data.value > 0" },
        ],
        transform: [
          { key: "transform", label: "Transform Script", type: "textarea", default: "return data;" },
        ],
        custom: [
          { key: "script", label: "Custom Script", type: "textarea", default: "// Write your custom logic here" },
        ],
      };
      return defs[type] || [];
    },
    ...
    runWorkflow() {
      ...
      nodes.forEach((node, i) => {
        setTimeout(() => {
          ...
          this.addLog("info", `Executing node: ${node.name}`);
          ...
          setTimeout(() => {
            ...
            this.addLog(
              ...
              `${node.name} ${success ? "completed" : "failed"}`
            );
            ...
            if (i === nodes.length - 1) {
              ...
              this.toast(
                success ? "success" : "error",
                success ? "Workflow complete" : "Workflow failed",
